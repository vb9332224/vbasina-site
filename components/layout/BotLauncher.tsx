/**
 * Floating-launcher для Telegram-бота @prland1_bot.
 * Открывает chat в Telegram (приложение или web).
 * Появляется на всех маркетинговых страницах в правом нижнем углу.
 */
const BOT_URL = "https://t.me/prland1_bot?start=site";

export function BotLauncher() {
  return (
    <a
      href={BOT_URL}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Связаться с командой в Telegram"
      className="
        fixed z-40
        bottom-20 right-4 md:bottom-6 md:right-6
        inline-flex items-center gap-2 md:gap-3
        bg-gold-500 hover:bg-gold-300 text-navy-900
        px-4 md:px-5 py-3 md:py-3.5
        shadow-xl
        text-sm md:text-[15px] font-semibold
        transition-colors
        border border-navy-900/10
      "
    >
      <svg
        width="20"
        height="20"
        viewBox="0 0 24 24"
        fill="currentColor"
        aria-hidden="true"
        className="shrink-0"
      >
        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 7.05l-1.55 7.31c-.12.52-.42.65-.85.4l-2.35-1.74-1.13 1.09c-.13.13-.24.24-.48.24l.17-2.43 4.42-4c.19-.17-.04-.27-.3-.1l-5.47 3.44-2.36-.74c-.51-.16-.52-.51.11-.75l9.21-3.55c.43-.16.81.1.66.83z" />
      </svg>
      <span className="whitespace-nowrap">Связаться с командой</span>
    </a>
  );
}
